...
contract FiitToken is Context, IBEP20, Ownable {
  ...
  */
  ...
  function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
    ...
    _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "FIIT: transfer amount exceeds allowance"));
    return true;
  }
  ...
  function decreaseAllowance(address spender, uint256 subtractedValue) public returns (bool) {
    _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "FIIT: decreased allowance below zero"));
    return true;
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    require(sender != address(0), "FIIT: transfer from the zero address");
    require(recipient != address(0), "FIIT: transfer to the zero address");

    _balances[sender] = _balances[sender].sub(amount, "FIIT: transfer amount exceeds balance");
  ...
  function _burn(address account, uint256 amount) internal {
    require(account != address(0), "FIIT: burn from the zero address");

    _balances[account] = _balances[account].sub(amount, "FIIT: burn amount exceeds balance");
    _totalSupply = _totalSupply.sub(amount);
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    require(owner != address(0), "FIIT: approve from the zero address");
    require(spender != address(0), "FIIT: approve to the zero address");
  ...
  function _burnFrom(address account, uint256 amount) internal {
    _burn(account, amount);
    _approve(account, _msgSender(), _allowances[account][_msgSender()].sub(amount, "FIIT: burn amount exceeds allowance"));
  }
}